...
async function getLeaderboard(
  ...
  startIdx: number = 0,
  pageSize: number = -1,
): Promise<ItemList<LeaderboardItem>> {
  const guessList: ItemList<GuessResult> = await axios.get(
...
function mapGuessResultToLocationImage(
...
function mapGuessResultToLeaderboardItem(
  guessResult: GuessResult,
): LeaderboardItem {
  return {
    id: guessResult.id,
...
async function getGuessesByUser(
  userId: string,
  startIdx: number,
  pageSize: number,
): Promise<ItemList<LocationImage>> {
  const userGuessList: ItemList<GuessResult> = await axios.get(
    `user/${userId}/guess`,
    {
      params: {
        startIdx,
